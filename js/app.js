...
import { AutoSave } from './supabase.js';
...
const app = {
    ...
    getOpIcon(id) {
        const icons = {
            ...
            inventory: '📦', customers: '👥', categories: '🏷️',
        ...
        let content = `
            <div class="card page-enter" style="max-width: 800px; margin: 0 auto; padding: 2rem;">
                <div class="card-header" style="text-align: center; display: block; padding-bottom: 2rem; border-bottom: 1px solid var(--border); margin-bottom: 2rem;">
                    ...
                    ${role === 'branch' ? `<div style="margin-top: 0.5rem; color: var(--text-muted); font-size: 0.9rem;">ID: ${profile.branch_login_id || profile.branch_id}</div>` : ''}
                </div>
        ...
        content += `
            <div class="settings-section">
                ...
                <div style="display: flex; align-items: center; justify-content: space-between;">
                   ...
                   <select id="settings-currency" class="input-field" style="width: auto; min-width: 120px;">
            ...
            if (role === 'admin') {
                ...
                if (secForm) {
                    secForm.addEventListener('submit', async (e) => {
                        ...
                        const btn = secForm.querySelector('button');
                        try {
                            btn.disabled = true;
                            btn.textContent = 'Updating...';
